...
void handle_page_fault(int signo, siginfo_t *si, void *context) {
    ...
    for (i = 0; i < ehdr->e_phnum; i++) {
        ...
        if (fault_addr >= start_addr && fault_addr <= end_addr) {
            ...
            void *mem = map_memory(phdr[i].p_vaddr, phdr[i].p_memsz, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS);
            ...
            int page_size = 4096;  // Page size in bytes (4 KB)
            int allocated_pages = phdr[i].p_memsz / page_size + 1;
            internal_fragmentation += allocated_pages * page_size - phdr[i].p_memsz;
            page_allocations += allocated_pages;
            break;
        }
    }
    return;
